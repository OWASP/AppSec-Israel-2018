...
<div class="keynote-full">
	...
	{% for member in site.data.keynotespeakers %}
		{% if member.name %}
		<li>
			<div class="image" style="background-image: url({{baseUrl}}assets/images/keynotes/{{member.image | default: '../placeholder.svg'}});{{member.style}};"></div>
			<div>
				<h1>{{member.name}}</h1>
				{% if member.uri %}
					<h3><a href="{{baseUrl}}{{member.uri}}">{{member.title}}</a></h3>
				{% else %}
					<h3>{{member.title}}</h3> 
				{% endif %}
				<p>
